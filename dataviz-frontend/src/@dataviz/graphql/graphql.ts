...
export function createApollo(httpLink: HttpLink): ApolloClientOptions<any> {
    ...
    const errorLink = onError(({ graphQLErrors, networkError }) => { // Removed 'response' as it's not used
        if (graphQLErrors) {
            graphQLErrors.forEach(({ message, extensions }) => { // Added 'extensions' for more detailed error handling
